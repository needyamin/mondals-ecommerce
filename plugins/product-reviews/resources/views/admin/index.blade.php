...
<div class="mb-8">
    ...
    <p class="text-slate-500 dark:text-slate-400 mt-1.5 text-sm max-w-2xl">Approve, reject, or remove customer reviews. Only <span class="font-semibold text-slate-700 dark:text-slate-300">approved</span> reviews appear on product pages.</p>
</div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-6">
    @foreach([
        'pending' => ['label' => 'Pending', 'sub' => 'Awaiting action', 'icon' => 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z'],
        'approved' => ['label' => 'Approved', 'sub' => 'Live on site', 'icon' => 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'],
        'rejected' => ['label' => 'Rejected', 'sub' => 'Hidden', 'icon' => 'M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728L5.636 5.636'],
        'all' => ['label' => 'All reviews', 'sub' => 'Total in system', 'icon' => 'M4 6h16M4 10h16M4 14h16M4 18h16'],
    ] as $key => $meta)
        @php $active = $cur === $key; @endphp
        <a href="{{ route('admin.reviews.index', array_filter(['status' => $key, 'search' => $searchQ])) }}"
           ...
           {{ $active
                ? 'border-brand-500/50 bg-brand-50/80 dark:bg-brand-900/15 dark:border-brand-500/40 shadow-sm ring-1 ring-brand-500/25'
                : 'border-slate-200/90 dark:border-slate-700 bg-white dark:bg-darkpanel hover:border-brand-200 dark:hover:border-slate-600 hover:bg-slate-50/80 dark:hover:bg-slate-800/40' }}">
            <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg {{ $active ? 'bg-brand-600 text-white shadow-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 group-hover:bg-brand-50 dark:group-hover:bg-slate-700 group-hover:text-brand-600 dark:group-hover:text-brand-400' }} transition-colors">
            ...
            <div class="min-w-0 flex-1 text-left">
                <div class="flex items-center gap-1.5 flex-wrap">
                    ...
                    @if($active)
                        <span class="text-[9px] font-bold uppercase tracking-wide text-brand-600 dark:text-brand-400 bg-brand-100/80 dark:bg-brand-900/40 px-1.5 py-0.5 rounded">Active</span>
                    @endif
                </div>
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-slate-50/90 to-white dark:from-slate-800/40 dark:to-darkpanel">
        <div class="flex flex-nowrap items-center justify-between gap-3 min-w-0 overflow-x-auto pb-0.5">
            <div class="flex items-center gap-2 sm:gap-3 min-w-0 shrink whitespace-nowrap">
                <h3 class="text-base sm:text-lg font-bold text-slate-900 dark:text-white font-heading shrink-0">Review list</h3>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 shrink-0">
                    ...
                    @if($searchQ)
                        <span class="text-brand-600 dark:text-brand-400"> · filtered</span>
                    @endif
                </p>
            </div>
            <form method="GET" action="{{ route('admin.reviews.index') }}" class="flex items-center gap-2 shrink-0">
                <input type="hidden" name="status" value="{{ $cur }}">
                <div class="flex w-[13rem] sm:w-56 min-w-0 items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 px-2.5 shadow-inner focus-within:ring-2 focus-within:ring-brand-500 focus-within:border-transparent">
                ...
                <button type="submit" class="shrink-0 px-3 py-2 rounded-xl bg-brand-600 hover:bg-brand-700 text-white text-xs font-bold shadow-md shadow-brand-500/25 transition">Search</button>
                @if($searchQ)
                    <a href="{{ route('admin.reviews.index', ['status' => $cur]) }}" class="shrink-0 px-2.5 py-2 rounded-xl border border-slate-200 dark:border-slate-600 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition">Clear</a>
                @endif
            </form>
        </div>
    </div>
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/20">
                    ...
                    <th class="px-6 py-4 min-w-[200px]">Comment</th>
                    <th class="px-6 py-4 text-center w-28">Status</th>
                    <th class="px-6 py-4 text-right pr-6 sm:pr-8 whitespace-nowrap w-40">Actions</th>
                </tr>
            </thead>
                ...
                @forelse($reviews as $review)
                    <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors align-top">
                        <td class="px-6 py-5 pl-6 sm:pl-8">
                            <a href="{{ route('product.detail', $review->product->slug) }}" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 font-bold text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 text-sm group/link">
                                <span class="group-hover/link:underline">{{ Str::limit($review->product->name, 42) }}</span>
                            ...
                            @if($review->product->vendor)
                                <p class="text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wide mt-1.5">{{ $review->product->vendor->store_name }}</p>
                            @endif
                        </td>
                        <td class="px-6 py-5">
                        ...
                        <td class="px-6 py-5 text-right pr-6 sm:pr-8 whitespace-nowrap">
                            <div class="inline-flex flex-nowrap justify-end items-center gap-1">
                                @if($review->status !== 'approved')
                                    <form action="{{ route('admin.reviews.approve', $review->id) }}" method="POST" class="inline shrink-0">
                                        @csrf
                                ...
                                @if($review->status !== 'rejected')
                                    <form action="{{ route('admin.reviews.reject', $review->id) }}" method="POST" class="inline shrink-0" onsubmit="return confirm('Reject this review?');">
                                        @csrf
                                ...
                                @endif
                                <form action="{{ route('admin.reviews.destroy', $review->id) }}" method="POST" class="inline shrink-0" onsubmit="return confirm('Delete this review permanently?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="px-2 py-1 rounded-md border border-rose-200 dark:border-rose-800/60 bg-rose-50 dark:bg-rose-900/20 text-rose-700 dark:text-rose-300 text-[10px] font-bold leading-none hover:bg-rose-100 dark:hover:bg-rose-900/40 transition">Delete</button>
                                </form>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="6" class="px-8 py-16 sm:py-20">
                            <div class="max-w-md mx-auto text-center">
                                ...
                                <p class="text-base font-bold text-slate-800 dark:text-slate-200">No reviews here</p>
                                <p class="text-sm text-slate-500 dark:text-slate-400 mt-2">
                                    @if($searchQ)
                                        Nothing matches your search. Try different keywords or
                                        <a href="{{ route('admin.reviews.index', ['status' => $cur]) }}" class="font-bold text-brand-600 dark:text-brand-400 hover:underline">clear search</a>.
                                    @else
                                        When customers leave reviews, they appear in <span class="font-semibold">Pending</span> until approved.
                                    @endif
                                </p>
                            </div>
                        </td>
