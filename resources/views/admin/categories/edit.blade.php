...
@section('content')
    ...
    <div class="mb-8 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-3xl font-extrabold text-slate-900 dark:text-white font-heading tracking-tight">Modify Category</h2>
            <p class="text-slate-500 dark:text-slate-400 mt-1">Editing classification: {{ $item->name }}</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.categories.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8 pb-10">
        @csrf
        @method('PUT')
                ...
                <div class="bg-white dark:bg-darkpanel p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="3" class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors">{{ old('description', $item->description) }}</textarea>
                        </div>
                    </div>
                </div>
                        ...
                        <div>
                            ...
                            <select name="parent_id" class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors">
                                ...
                                @foreach($parents as $id => $pName)
                                    <option value="{{ $id }}" {{ old('parent_id', $item->parent_id) == $id ? 'selected' : '' }}>{{ $pName }}</option>
                                @endforeach
                            </select>
                        </div>
    ...
    <style>
        ...
        input:checked ~ .target-bg { background-color: #4f46e5; }
    </style>
