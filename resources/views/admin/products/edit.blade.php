...
@section('content')
    ...
    <div class="mb-8 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-3xl font-extrabold text-slate-900 dark:text-white font-heading tracking-tight">Modify Inventory</h2>
            <p class="text-slate-500 dark:text-slate-400 mt-1">Editing sku: {{ $item->sku }}</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.products.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8 pb-10">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white dark:bg-darkpanel p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $item->name) }}" required class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors">
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Short Description</label>
                            <textarea name="short_description" rows="2" class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors">{{ old('short_description', $item->short_description) }}</textarea>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Full Description</label>
                            <textarea name="description" rows="5" class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors">{{ old('description', $item->description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-darkpanel p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" step="0.01" name="price" value="{{ old('price', collect($item->price)->first() ?: $item->price) }}" required class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Compare Price (৳)</label>
                            <input type="number" step="0.01" name="compare_price" value="{{ old('compare_price', collect($item->compare_price)->first() ?: $item->compare_price) }}" class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors">
                        </div>
                        <div>
                            ...
                            <input type="number" name="quantity" value="{{ old('quantity', $item->quantity) }}" required class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">SKU Code <span class="text-rose-500">*</span></label>
                            <input type="text" name="sku" value="{{ old('sku', $item->sku) }}" required class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors uppercase font-mono tracking-widest">
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-8">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <select name="status" class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors">
                                <option value="approved" {{ old('status', $item->status) === 'approved' ? 'selected' : '' }}>Approved / Live</option>
                                <option value="pending" {{ old('status', $item->status) === 'pending' ? 'selected' : '' }}>Pending Review</option>
                                <option value="draft" {{ old('status', $item->status) === 'draft' ? 'selected' : '' }}>Draft Mode</option>
                        ...
                        <div>
                            ...
                            <select name="vendor_id" required class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors">
                                ...
                                @foreach($vendors as $id => $name)
                                    <option value="{{ $id }}" {{ old('vendor_id', $item->vendor_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-4">
                            <label class="flex items-center cursor-pointer group">
                                <div class="relative">
                                    <input type="checkbox" name="is_active" value="1" class="sr-only" {{ old('is_active', $item->is_active) ? 'checked' : '' }}>
                            ...
                            <label class="flex items-center cursor-pointer group">
                                <div class="relative">
                                    <input type="checkbox" name="is_featured" value="1" class="sr-only" {{ old('is_featured', $item->is_featured) ? 'checked' : '' }}>
                                    <div class="block w-10 h-6 bg-slate-200 dark:bg-slate-700 rounded-full transition-colors target-bg"></div>
                                    <div class="dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform"></div>
                                </div>
                ...
                <div class="bg-white dark:bg-darkpanel p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 flex justify-center sticky top-24">
                    <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 px-8 rounded-xl shadow-lg shadow-indigo-500/30 transition-all hover:-translate-y-1">
